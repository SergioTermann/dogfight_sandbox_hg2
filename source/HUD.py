...
class HUD_Aircraft:
	...
	def update(cls, Main, aircraft: Aircraft, targets):
		...
		# 绘制加速度指示器（垂直条）
		acc = aircraft.get_linear_acceleration()
		max_acc_display = 10.0  # 最大显示加速度范围
		
		# 加速度条的位置和尺寸
		bar_x = 0.02  # 左侧位置
		bar_y_center = 0.5  # 屏幕中央
		bar_width = 0.015
		bar_height = 0.3
		
		# 背景框
		Overlays.add_text2D("│", hg.Vec2(bar_x, bar_y_center - bar_height/2), 0.5, hg.Color(0.3, 0.3, 0.3, 0.7) * f, Main.hud_font)
		Overlays.add_text2D("│", hg.Vec2(bar_x, bar_y_center + bar_height/2), 0.5, hg.Color(0.3, 0.3, 0.3, 0.7) * f, Main.hud_font)
		
		# 零点标记
		Overlays.add_text2D("━", hg.Vec2(bar_x - 0.005, bar_y_center), 0.025, hg.Color(1, 1, 1, 0.5) * f, Main.hud_font)
		
		# 加速度指示（根据正负方向）
		acc_normalized = max(-1, min(1, acc / max_acc_display))  # 归一化到-1到1
		bar_pos_y = bar_y_center + acc_normalized * (bar_height / 2)
		
		# 根据加速度选择颜色
		if abs(acc) < 2:
			bar_color = hg.Color(0.5, 1, 0.5, 1)  # 绿色
		elif abs(acc) < 5:
			bar_color = hg.Color(1, 1, 0, 1)  # 黄色
		elif abs(acc) < 8:
			bar_color = hg.Color(1, 0.6, 0, 1)  # 橙色
		else:
			bar_color = hg.Color(1, 0.2, 0.2, 1)  # 红色
		
		# 绘制加速度指示器（三角形箭头）
		if acc > 0.1:
			Overlays.add_text2D("▲", hg.Vec2(bar_x - 0.007, bar_pos_y), 0.025, bar_color * f, Main.hud_font)
		elif acc < -0.1:
			Overlays.add_text2D("▼", hg.Vec2(bar_x - 0.007, bar_pos_y), 0.025, bar_color * f, Main.hud_font)
		else:
			Overlays.add_text2D("●", hg.Vec2(bar_x - 0.005, bar_pos_y), 0.020, bar_color * f, Main.hud_font)
		
		# 在指示器旁边显示具体数值
		Overlays.add_text2D("%.1f" % acc, hg.Vec2(bar_x + 0.025, bar_pos_y - 0.01), 0.015, bar_color * f, Main.hud_font)
		
		# 标签和刻度
		Overlays.add_text2D("ACC", hg.Vec2(bar_x - 0.01, bar_y_center - bar_height/2 - 0.03), 0.012, hg.Color.White * f, Main.hud_font)
		Overlays.add_text2D("+10", hg.Vec2(bar_x + 0.015, bar_y_center - bar_height/2), 0.010, hg.Color(0.5, 1, 0.5, 0.5) * f, Main.hud_font)
		Overlays.add_text2D("0", hg.Vec2(bar_x + 0.015, bar_y_center - 0.01), 0.010, hg.Color(1, 1, 1, 0.5) * f, Main.hud_font)
		Overlays.add_text2D("-10", hg.Vec2(bar_x + 0.015, bar_y_center + bar_height/2), 0.010, hg.Color(1, 0.5, 0.5, 0.5) * f, Main.hud_font)
